...
# --- CLASS WRAPPER CHO HUGGING FACE VITPOSE ---
...
while cap.isOpened():
    ...
    cluster_results, groups_status, max_group_id = cluster_bboxes_with_ids(
        ...
        pose_vectors=pose_vectors_list,
    ...
    overlay = frame.copy()
    ALPHA_FOV = 0.3  # Độ mờ 30%
    FOV_RADIUS = 75  # Độ vươn xa của tầm nhìn (pixel)
    FOV_ANGLE = 60   # Góc mở rộng của tầm nhìn (60 độ)
    ...
    for person in cluster_results:
        ...
        if tid in active_tracks_info:
            ...
            if info['pose_vec'] is not None:
                px, py = info['pose_vec']
                base_angle = math.degrees(math.atan2(py, px))
                
                cv2.ellipse(overlay, (cx, cy), (FOV_RADIUS, FOV_RADIUS), 
                            0, base_angle - FOV_ANGLE/2, base_angle + FOV_ANGLE/2, 
                            color, -1)

    # Trộn lớp overlay vào frame
    cv2.addWeighted(overlay, ALPHA_FOV, frame, 1 - ALPHA_FOV, 0, frame)


    # [NHỊP 2]: VẼ CÁC CHI TIẾT NÉT CỨNG
    for person in cluster_results:
        bbox = person['bbox']
        color = person['color']
        tid = person['id_p']
        ...
        if tid in active_tracks_info:
            ...
            if 'keypoints' in info and info['keypoints'] is not None:
                kpts = info['keypoints']
                CONF_THRESH = 0.4 
                ...
                SKELETON_LINKS = [
                    ...
                    ((2, 4), (0, 0, 255)),                            
                    ((5, 6), (255, 255, 0)), ((11, 12), (255, 255, 0)), 
                    ((5, 11), (255, 255, 0)), ((6, 12), (255, 255, 0)), 
                    ((0, 1), (255, 255, 0)), ((0, 2), (255, 255, 0))    
                ]
                
                for (pt1_idx, pt2_idx), link_color in SKELETON_LINKS:
                    if kpts[pt1_idx][2] > CONF_THRESH and kpts[pt2_idx][2] > CONF_THRESH:
                        x1, y1 = int(kpts[pt1_idx][0]), int(kpts[pt1_idx][1])
                        x2, y2 = int(kpts[pt2_idx][0]), int(kpts[pt2_idx][1])
                        cv2.line(frame, (x1, y1), (x2, y2), link_color, 2)
    ...
    if homography_matrix is not None:
        ...
        for i in range(num_people):
            for j in range(i + 1, num_people):
                ...
                # Chỉ vẽ khoảng cách giữa 2 người CÙNG NHÓM
                if p1['id_g'] != -1 and p1['id_g'] == p2['id_g']:
                    ...
                    if pos1 is not None and pos2 is not None:
                        ...
                        cv2.line(frame, c1, c2, p1['color'], 2) 
                        
                        mid = ((c1[0]+c2[0])//2, (c1[1]+c2[1])//2)
                        dist_text = f"{dist:.1f}m"
                        (tw, th), _ = cv2.getTextSize(dist_text, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)
                        cv2.rectangle(frame, (mid[0], mid[1]-th-2), (mid[0]+tw, mid[1]+4), (0,0,0), -1)
                        cv2.putText(frame, dist_text, (mid[0], mid[1]+2), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
